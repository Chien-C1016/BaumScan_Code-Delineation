...
threshold_intermodes = "threshold_intermodes";
Ext.CLIJ2_thresholdIntermodes(top_hat_r200, threshold_intermodes);

// ** Some cases, use direct Ostu filter is better ...
// Ext.CLIJ2_thresholdOtsu(top_hat_r200, threshold_intermodes);

// Pull the blurred image back to CPU memory
Ext.CLIJ2_pull(threshold_intermodes);

// Show the result
// run("Tile");

// Remove the padding using ImageJ's crop function
selectWindow("threshold_intermodes");
makeRectangle(paddingSize, paddingSize, width, height);
run("Crop");

// Push the cropped image back to GPU memory
Ext.CLIJ2_push(threshold_intermodes);
...
Ext.CLIJ2_closingBox(threshold_intermodes, closingImage, number_of_iterations);
