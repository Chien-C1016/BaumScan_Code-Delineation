...
setwd("C:/Users/chien/OneDrive - TUM/PhD PaperWriting/Paper_01_Automatic pith and tree-ring detection/Protocol/Printer Scanner")
...
# Data Input ----
# Image Import
im  = OpenImageR::readImage("05_invert.tif") # Median + invert Processed
...
# Edge Import
im.imj  = OpenImageR::readImage("06_Thres_Otsu.tif")
...
eps_scale <- 10 #dat %>% dbscan.eps()
frnn_dat  <- dbscan::frNN(dat, eps = eps_scale)
db_dat    <- dbscan::dbscan(frnn_dat, minPts = 20)
...
#'[Perform_dbscan:]
eps_scale    <- 5 #df.edge %>% dbscan.eps()
frnn_df.edge <- dbscan::frNN(df.edge, eps = eps_scale)
db_df.edge   <- dbscan::dbscan(frnn_df.edge, minPts = 3)
...
#' As NO Fresh Sapwood is there influencing the clearness,
#' The control should apply general or even stricter than CT.
sapw.control <- rep(0.6, (length(clst.trust)))
